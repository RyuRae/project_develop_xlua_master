...
namespace FileTools
{
    public class AssetRecord
    {
        ...
        public AssetRecord(FileInfo fileInfo, string mainManifestName = null) : this()
        {
            ...
            _MD5Value = MD5Helper.md5file(fileInfo.FullName);
        }

        /// <summary>
        /// 设置资源的hash值
        /// </summary>
        public void SetResHash(string hash)
        {
            _MD5Value = hash;
        }
        ...
        public virtual void ReadByBinaryReader(BinaryReader br)
        {
            ...
            _MD5Value = FileHelper.ReadBinaryString(br);
        }
        ...
        public virtual void WriteByBinaryWriter(BinaryWriter bw)
        {
            ...
            bw.Write(Version);
            FileHelper.WriteBinaryString(bw, MD5Value);
        }

        #endregion
        // ---------------------------------------------------------------------------------
        #region bundleName的命名规则和解析规则，如果要改，两个函数应该一起改
